...
CLUSTERS=$(gcloud workstations clusters list --project="$PROJECT" --format='value(name)' 2>/dev/null)
if [ -z "$CLUSTERS" ]; then
  ...
  for region in us-central1 us-east1 us-west1 us-south1 europe-west1; do
    CLUSTERS+=$(gcloud workstations clusters list --project="$PROJECT" --region="$region" \
...
if [ -z "$CLUSTERS" ]; then
  ...
  while IFS= read -r cluster_path; do
    ...
    CONFIGS=$(gcloud workstations configs list --project="$PROJECT" --region="$region" \
      --cluster="$cluster" --format='value(name)' 2>/dev/null | sed 's|.*/||')
    if [ "${DELETE_IDLE:-}" = "1" ]; then
      for config in $CONFIGS; do
        for ws in $(gcloud workstations list --project="$PROJECT" --region="$region" \
                      --cluster="$cluster" --config="$config" --format='value(name)' 2>/dev/null | sed 's|.*/||'); do
      ...
      if gcloud workstations clusters delete "$cluster" --project="$PROJECT" --region="$region" \
        ...
        warn "  gcloud workstations clusters delete $cluster --region=$region --project=$PROJECT --quiet"
      fi
    else
