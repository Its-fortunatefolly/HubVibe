...
`go-live.sh` now defaults to Dexter; the one-variable swap the handoff
always described is now literally one variable. (`go-live-x402.sh` is
superseded — it now only points at `go-live.sh`.)
...
```bash
cd ~/HubVibe-deploy4 && git fetch origin main && git reset --hard origin/main
RAILS=x402 bash scripts/go-live.sh   # sets X402_FACILITATOR_URL=https://x402.dexter.cash + deploys
```
...
before #57. `go-live.sh` ends with a source deploy. Do not shortcut it.
