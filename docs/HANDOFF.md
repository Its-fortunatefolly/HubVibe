...
- **First paid call:** NOT yet made — **only the money is missing.** The
  ...
  `first-paid-call.sh --new-wallet`, its key at **`~/.hubvibe-wallet-key`**
  (every script reads that path; an earlier line here said
  `~/.hubvibe-payer-key`, which is a file that does not exist and sent the
  owner looking for a wallet they could not find). Its last attempt was
  refused `invalid_exact_evm_insufficient_balance`: the EIP-3009 signature
  was VALID and every step up to the balance is proven. Fund it with USDC
  **on Base** and re-run. There is no recovery phrase for the owner's own
  wallet — do not build for one.
- **That payer address is a WALLET, not an account id.** It was generated by
  `eth_account` on the box and its private key is `~/.hubvibe-wallet-key`
  there, mode 600. Anything sent to it is the owner's and is recoverable
  with that file. It exists identically on every EVM chain, so USDC sent on
  Ethereum, Arbitrum, Optimism or Polygon by mistake is sitting at the same
  address on that chain — not lost, just not on Base.
- **The balance check reads ONE token on ONE chain:** native USDC
  ...
  exactly like "never received". Check the address on a multi-chain explorer
  (`https://blockscan.com/address/<addr>`) before believing money is missing.
