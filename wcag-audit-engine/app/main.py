...
def _paid_route_price(path: Optional[str]) -> Optional[float]:
    ...
    # Not an audit: the worker network prices its own routes from its own
    # catalog. Checked only after _CATALOG, so a worker can never shadow or
    # alter what an audit route charges.
    if workers is not None:
        return workers.catalog.price_of(resolved)
    return None
